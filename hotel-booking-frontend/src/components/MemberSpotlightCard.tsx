...
const MemberSpotlightCard: FC = () => {
  return (
    <article className="relative isolate flex min-h-[320px] w-full flex-col justify-end overflow-hidden rounded-[42px] border border-white/8 bg-black/10 p-8 text-white shadow-[0_40px_140px_rgba(3,2,20,0.65)]">
      <img
        src={spotlightCard.heroImage}
        alt={spotlightCard.locationLabel}
        loading="lazy"
        className="absolute inset-0 h-full w-full object-cover object-center brightness-[1.35] contrast-[1.15] saturate-[1.35]"
      />
      <div className="absolute inset-0 bg-gradient-to-br from-black/10 via-transparent to-transparent" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(140,161,255,0.35),_transparent_65%)]" />
      <div className="absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-[#040619]/60 via-transparent to-transparent" />
      ...
      <div className="absolute right-4 top-4 rounded-[26px] border border-white/15 bg-black/50 px-5 py-4 text-right text-white/90 backdrop-blur-xl">
